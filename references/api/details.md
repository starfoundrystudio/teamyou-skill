...
- [List details for a topic](#list-details-for-a-topic)
- [Add details (batch, max 50)](#add-details-batch-max-50)
- [Update a detail](#update-a-detail)
- [Delete a detail](#delete-a-detail)

### List details for a topic

```http
GET /topics/{id}/details
```

Returns an empty list (200) if the topic is missing or not owned — does not 404.

**Skill CLI:** `teamyou.sh ty graph details-list <topic_id>`
...
### Add details (batch, max 50)

```http
POST /topics/{id}/details
```

**Skill CLI:** `teamyou.sh ty graph details-add <topic_id> <detail1> [detail2] [detail3] ...`
...
| --- | --- | --- | --- |
| `details` | object[] | yes | max 50 items |

**Responses:**

- `201` — Created details (each embedded for semantic search).
...
```http
PUT /topics/{id}/details/{detailId}
```

**Skill CLI:** `teamyou.sh ty graph details-update <topic_id> <detail_id> <text>`

**Parameters:**

- `id` (path, required) — string — Topic id
- `detailId` (path, required) — string — Detail id

**Request body:**

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `detail` | string | yes | len 1..5000 |
...
```http
DELETE /topics/{id}/details/{detailId}
```

**Skill CLI:** `teamyou.sh ty graph details-delete <topic_id> <detail_id>`
