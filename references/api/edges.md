...
- [List edges touching a topic](#list-edges-touching-a-topic)
- [Create an edge](#create-an-edge)
- [Update edge labels](#update-edge-labels)
- [Delete an edge](#delete-an-edge)

### List edges touching a topic

```http
GET /edges
```

**Skill CLI:** `teamyou.sh ty graph edges-list <topic_id>`

**Parameters:**

- `topicId` (query, required) — string — Topic id (required).

**Responses:**

- `200` — Edges normalized to the topic perspective.
...
201 when created; 200 when an inverse-direction equivalent already exists (idempotent). Creation does not require the AI-update preference.

**Skill CLI:** `teamyou.sh ty graph edges-create <source_topic_id> <target_topic_id> <label> [--mirror-label <text>]`
...
| --- | --- | --- | --- |
| `sourceTopicId` | string | yes | len 1..∞ |
| `targetTopicId` | string | yes | len 1..∞ |
| `label` | string | yes | len 1..255 |
| `mirrorLabel` | string | no | len 1..255 |

**Responses:**

- `200` — Idempotent — existing inverse edge returned.
- `201` — Newly created edge.
- `400` — Validation error — invalid body, query, or path param (code: validation_error or invalid_json). `details` carries Zod field errors; PATCH /edges also adds `formErrors`.
- `401` — Missing, malformed, expired, or revoked API key (code: unauthorized).
...
**Skill CLI:** `teamyou.sh ty graph edges-update <edge_id> [--label <text>] [--mirror-label <text>]`

**Parameters:**

- `id` (path, required) — string — Edge id

**Request body:**

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `label` | string | no | len 1..255 |
| `mirrorLabel` | string | no | len 1..255 |
...
**Skill CLI:** `teamyou.sh ty graph edges-delete <edge_id>`
